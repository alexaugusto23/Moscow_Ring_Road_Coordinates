...
def form(origin: str = None, destiny: str = None ) -> str:
    ...
    url = f"https://maps.googleapis.com/maps/api/distancematrix/json?origins={origin}&destinations={destiny}&key={API_KEY}"
    print(f"Origin:{type(origin)}/nDestiny:{type(destiny)}")

    try:
        count = 0
        for i in range(len(mkad_km)):
            if (float(origin[0]) and float(origin[1]) in mkad_km[i]) or (float(destiny[0]) and float(destiny[1]) in mkad_km[i]):
                        count += 1 
        if count >= 2:
            status = True
            distancia = "Inside of Mkad"
    ...
    if status == True:
        ...
        if maps['status'].lower() == 'ok':
            try: 
                ...
                distancia =  str(maps['rows'][0]['elements'][0]['status'])
            return render_template('index_mkad.html',distancia=distancia) 
        else:
            return render_template('index_mkad.html',distancia='Not found')
